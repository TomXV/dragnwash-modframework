...
{
    // Two ways in (docs/LAUNCHER_APP.md):
    //
    ...
    internal static class Program
    {
        ...
        private static int Main(string[] args)
        {
            ...
            var game = new Game(afterExit ? new string[0] : args);
            string folder = GameFolder(here, afterExit ? null : args.FirstOrDefault());
            Log.Open(folder != null ? Path.Combine(folder, "BepInEx", Paths.InstallerFolder) : here);
            Log.Line($"Launcher {typeof(Program).Assembly.GetName().Version.ToString(3)}: {(afterExit ? string.Join(" ", args) : args.Length == 0 ? "no command" : "command " + args[0] + (args.Length > 1 ? $" and {args.Length - 1} arguments" : ""))}");
            ...
            {
                run = afterExit ? AfterExit(args, folder) : Passthrough(game, folder);
                run.ContinueWith(t =>
                {
                    if (t.IsFaulted)
                    {
        ...
        private static async Task AfterExit(string[] args, string folder)
        {
            ...
            Task gone = int.TryParse(Arg("--wait-pid"), out int pid) ? Game.WaitForPid(pid) : Task.CompletedTask;
            bool again = true;
            try
            {
                if (folder == null)
                ...
                {
                    var files = new GameFiles(folder);
                    IEnumerable<string> guids = Arg("--mods")?.Split(',').Select(g => g.Trim()).Where(g => g.Length > 0);
