...
- Install.exe is still one file. The WebView2 DLLs are inside it, and `WebView2Loader.dll` is written once to `%LOCALAPPDATA%\DragNWash ModFramework\Installer` and checked by its SHA-256 before every use; nothing is written next to the exe. It's about 1.7 MB now, and still built byte for byte the same from the same sources.
