...
**チェックボックス。** 「インストール」の枠に「起動時に更新を確認する（Steam の起動オプションを設定）」があり、最初から入っています。「インストールですること」には「Steam のゲームの起動オプションに、更新用のランチャーを足す（前からある起動オプションはそのまま）」か、もう入っていることが出ます。PC に Steam のアカウントがないときと、起動するランチャーがないとき（1.6.0 より古いフレームワークを指定している Mod）は、灰色で押せません。
