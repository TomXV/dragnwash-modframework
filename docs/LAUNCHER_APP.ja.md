...
3. あるときは、ロゴが上に動いて更新の一覧が出ます。Mod ごとに今の版と新しい版、大きさ、変更点です。入れたいものにチェックを入れて、「更新して起動」で入れてから起動、「このまま起動」で何もせずに起動します。どちらも、窓を閉じてから起動します。「このバージョンは知らせない」にすると、その Mod のその版では、もうこの画面を出しません。次の版が出たら、また知らせます。その行は少し薄くなり、「知らせない」の小さな札がつきます。チェックはそのままなので、いま「更新して起動」を押せば入ります。
...
閉じ方はどの画面でも同じです。ランチャーが `bye` を送ると、ボードの中身が（カウントダウンのカードから先に）消え、ページも消えていき（全部で 0.3 秒）、`gone` を返します。ランチャーは窓そのものを消して（0.16 秒）隠し、それからゲームを起動します。「アニメーションを減らす」がオンなら、窓はそのまま消えます。

ランチャーの作業は絵よりずっと先に進むことがあるので、ページは知らせを順に並べ、手順ごとにデザインで決めた時間は必ず見せます。

手順にはそれぞれ、ロゴの横に絵があります。「入れる」では、いま入れている Mod のアイコンが出ます（`installing` で知らされた Mod を、それぞれ 1.4 秒以上。この知らせがないときは、選んだ Mod のアイコンを 1 つずつ全部見せます）。アイコンのない Mod は文字のタイルで、Mods 画面と同じく、名前の頭の「Drag'n Wash」を取って作ります。起動オプションの切り替えにも、Steam を閉じる絵と、起動オプションに足す絵・外す絵があります。失敗の画面のマークは、何があったかを表します。GitHub につながらないときは GitHub のマーク、ほかの失敗にもそれぞれの絵があり（返事がない、利用制限、GitHub に見つからない、ダウンロードが合わない、元に戻した、元に戻しきれなかった、ゲームがまだ動いている、別のローダー、ファイルを書き換えられない、Steam が閉じない）、それ以外は「！」です。

画面の切り替わり方は数種類で、どれも `webui/motion.js` が受け持ちます。先へ進むとき（「更新して起動」「もう一度試す」）は、前のボードの中身が左へ抜け、次のボードが右から入ってきます。「キャンセル」はその逆向きで、ロゴがヘッダーへ飛んで戻ります。失敗したときは、前のボードが静かに下へ消えてから、失敗の画面が入ってきます。カウントダウンのカードは、少しだけ行き過ぎて止まります。ランチャーへの知らせは押した瞬間に送り、動きが終わるのは待ちません。押したボードは、それ以上クリックを受けません。「アニメーションを減らす」がオンなら何も動かず、ボードは 0.12 秒で消えて 0.15 秒で出る、を順に行います。
