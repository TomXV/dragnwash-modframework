...
ゲームは `BepInEx/cache/DragNWash.ModFramework/update-request.json` を書いて終了します。ゲームが終わると、ランチャーがそのファイルを見つけ（今回の起動のあとに書かれたものだけを使い、読んだら消します）、更新の画面を出して入れ、3 秒数えたら窓を閉じて、同じコマンドでゲームをもう一度起動します。「今は起動しない」を押すと、起動せずに窓を閉じます。
