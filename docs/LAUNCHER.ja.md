...
- **ほかの起動のしかた**（起動オプションなしで Steam から、または exe から）：ゲームは `Launcher.exe --update-after-exit --wait-pid <pid> --mods "<guid>"` を窓なしで起動してから終了します。ランチャーはゲームが閉じるのを待って Mod を更新し、Steam からゲームを起動し直します。
