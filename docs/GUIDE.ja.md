...
| プレイヤーに設定を用意する | BepInEx の `Config.Bind`（Mods 画面が設定ページを作ります）、ゲームの Options 画面には `GameOptions.AddChoice` / `AddToggle` / `AddSlider` | 独自の設定メニュー |
...
2. **パッチの前に確認する。** 自分でゲームのメソッドにパッチが必要なときは、先にメソッドを探し、`GameHooks.Require(自分のGUID, "機能名", method != null, "Type.Method")` を呼びます。ゲームのアップデートで消えていたらその機能は使わずに済ませ、Mods 画面に「使えない機能」として表示されます。ゲームはクラッシュしません。`Require` は Harmony と同じ `"Type:Method"` の 1 つの文字列も受け取ります（`GameHooks.Require(自分のGUID, "機能名", "Yarn.Unity.LinePresenter:RunLine")`）。これは Inspector の Code 表示の **Copy** がコピーする形そのものなので、ゲーム内で見つけた名前をそのまま貼れます。**Patch** ボタンなら、この確認・`[HarmonyPatch]` 属性（名前だけでは曖昧なときは引数の型も）・Harmony が渡してくれる引数を持つ Prefix と Postfix を、一式でコピーできます
...
7. **ゲームのデータをそのまま配布しない。** ゲームから取り出したままのアセット、台本の文章、ゲームの DLL を、リポジトリやリリースに入れないでください。手で作ったものや、手を加えて新しくしたものは [docs/CONTENT_POLICY.ja.md](CONTENT_POLICY.ja.md) に従います
