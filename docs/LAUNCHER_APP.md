...
Closing goes the same way on every screen: the launcher sends `bye`, the board's parts go (the countdown card first) and the page fades out (0.3 s in all), the page answers `gone`, the launcher fades the window itself away (0.16 s) and hides it, and only then starts the game. With reduce-animations on, the window just goes.
...
Every step has its own picture beside the logo. Installing shows the icon of the mod going in (the one `installing` names, each for at least 1.4 s; without those messages the page shows every chosen mod's icon once), or its letter tile when it has none: the same tile as the Mods screen, from the name without a leading "Drag'n Wash". The launch option has its own pictures for closing Steam and for the option going in or coming out. A failure's mark says what went wrong: GitHub's mark when it couldn't be reached, a picture for each of the other kinds (no answer, the limit, not on GitHub, a download that doesn't match, put back, not all put back, the game still running, another loader, the file that can't be written, Steam not closing), and "!" for anything else.

Screens change in one of a few ways, all from `webui/motion.js`: onwards (**Update and play**, **Try again**) the old board's parts leave to the left and the next board's come in from the right; **Cancel** goes back the other way while the logo flies back up into the header; a failure leaves calmly downwards and the failed board only comes in once it's gone; the countdown card settles in with a slight overshoot. What's sent to the launcher goes on the click, never after the animation, and the board that was pressed takes no more clicks. With reduce-animations on, nothing moves: the old board fades out in 0.12 s, then the next one fades in over 0.15 s.
...
**Still one file.** Mods put Install.exe next to their files, and it runs from wherever the zip was extracted, so it can't bring DLLs along. The two WebView2 DLLs are resources in the exe, loaded from memory. `WebView2Loader.dll` (native, one for each of x64, x86 and Arm64) is a resource too: it's written once to `%LOCALAPPDATA%\DragNWash ModFramework\Installer\WebView2Loader\<processor>-<hash>` and its SHA-256 is checked before every use, so a damaged copy is written again. Nothing is written next to the exe. The page's own data goes in `%LOCALAPPDATA%\DragNWash ModFramework\Installer\WebView2`. That makes Install.exe about 1.8 MB instead of 180 KB, and it's still built byte for byte the same from the same sources.
