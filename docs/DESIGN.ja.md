...
状態: 2026 年 9 月。中核は 1.1.0、ライブラリは 1.0.0 としてリリースしました。予定や将来と書いた部分はまだ実装していません。どの部分についても、Issue で議論してください。
...
- インストーラー：このリポジトリに共通のインストーラーを置き、各 Mod の zip に同梱して、Mod ごとの小さな情報ファイルを読む形に決定（2026-09-15）
